...
class LatticeWithLongTie(Population):
    ...
    def __init__(self, m=None, n=None, copy_from=None):
        ...
        # self.init_longtie()
        self.dynamics = np.empty(len(self), dtype=int)
        self.dist = None
        self.category = 0
    ...
    def init_dynamics(self, adapter):
        ...
        self.dist = [(self.dynamics == m).sum() for m in range(adapter.category)]
    ...
    def init_longtie(self):
        ...
        long_tie = np.random.randint(size, size=size, dtype=int)
        ...
        # random
        self.long_tie = np.random.randint(size, size=size, dtype=int)
